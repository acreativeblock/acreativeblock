...
(function(){
  ...
  (function(){var refs=[].slice.call(document.querySelectorAll('.refpanel ol.refs>li'));if(!refs.length)return;var map={};refs.forEach(function(li,i){map[i+1]=li.textContent.replace(/\s+/g,' ').trim()});var tip;function hide(){if(tip){tip.remove();tip=null}}[].slice.call(document.querySelectorAll('sup.cite')).forEach(function(sup){var nums=sup.textContent.split(',').map(function(n){return n.trim()}).filter(Boolean);var texts=nums.map(function(n){return map[n]?(n+'. '+map[n]):null}).filter(Boolean);if(!texts.length)return;sup.classList.add('cite-tip');sup.setAttribute('tabindex','0');function show(){hide();tip=document.createElement('span');tip.className='cite-tip-bubble';tip.textContent=texts.join('  ·  ');document.body.appendChild(tip);var r=sup.getBoundingClientRect();var top=r.top+window.scrollY-tip.offsetHeight-10;var left=r.left+window.scrollX+r.width/2-tip.offsetWidth/2;left=Math.max(8,Math.min(left,window.scrollX+document.documentElement.clientWidth-tip.offsetWidth-8));tip.style.top=top+'px';tip.style.left=left+'px';requestAnimationFrame(function(){if(tip)tip.classList.add('is-visible')})}sup.addEventListener('mouseenter',show);sup.addEventListener('focus',show);sup.addEventListener('mouseleave',hide);sup.addEventListener('blur',hide)});document.addEventListener('scroll',hide,true)})();
