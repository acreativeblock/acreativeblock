/* Self-contained three-line ACB logo.
   The pupil follows the cursor and stays inside the O. */
(function(){
  /* Older questionnaire markup still calls a blink helper. Keep its logo static
     apart from the shared cursor-following pupil. */
  if(document.body&&document.body.classList.contains('questionnaire-page')&&window.SVGElement&&SVGElement.prototype.animate){
    var nativeSvgAnimate=SVGElement.prototype.animate;
    SVGElement.prototype.animate=function(){
      if(this.closest&&this.closest('.quiz-logo'))return{cancel:function(){},finish:function(){},play:function(){},pause:function(){}};
      return nativeSvgAnimate.apply(this,arguments);
    };
  }
  ...
  try{
    ...
    function wrap(node){
      [].slice.call(node.childNodes).forEach(function(k){
        if(k.nodeType===3){
          ...
          if(k.classList&&k.classList.contains('hl')){k.style.setProperty('--hlp','0');wrap(k);}
          else wrap(k);
        }
      });
    }
    ...
    function update(){
      ...
      nodes.forEach(function(el){
        ...
        el._hl.forEach(function(h){
          var hr=h.getBoundingClientRect();
          var hp=(start-hr.top)/(start-end);hp=Math.min(1,Math.max(0,hp));
          h.style.setProperty('--hlp',hp.toFixed(3));
        });
      });
    }
